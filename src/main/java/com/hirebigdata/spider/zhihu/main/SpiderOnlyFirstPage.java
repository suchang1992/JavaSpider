 ...
 */
public class SpiderOnlyFirstPage {
    ...
    public String spiderContent(String UID) throws MongoException {
        ...
        if (IDandName[0] != null && !IDandName[2].equals("404")) {
            ...
            FutureTask<ZhihuUserFollower> followersTasks = null;
            if(array[4]>=1){
                followersTasks = new FutureTask<ZhihuUserFollower>(new CallableUserFollower(
                        user.getUser_data_id(),
                        user.getUrl_name(),
                        user.getUser__xsrf_value(),
                        0
                ));
                pool.submit(followersTasks);
            }
            if(array[5]>=1)
...
//                }
                ...
                if(array[4]>=1){
                    ...
                    try {
                        zhihuUserFollower.getFollowers().addAll(followersTasks.get(1, TimeUnit.MINUTES).getFollowers());
                    } catch (InterruptedException e) {
                        e.printStackTrace();
