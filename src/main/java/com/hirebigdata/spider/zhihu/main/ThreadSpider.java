...
import com.hirebigdata.spider.zhihu.ThreadClass.*;
import com.hirebigdata.spider.zhihu.pojo.ZhihuUser;
import com.hirebigdata.spider.zhihu.pojo.ZhihuUserAnswer;
...
public class ThreadSpider {
    ...
    public String spiderContent(String UID) {
        ...
        if (IDandName[0] != null && !IDandName[2].equals("404")) {
            ...
            //创建线程
            pool.execute(new ThreadUserDetil(user));//线程加入 线程池中
            pool.execute(new ThreadUserSkill_Topic(IDandName[2], user.getUser_data_id()));
            if(array[2]>=1)
                pool.execute(new ThreadUserTopic(user.getUser_data_id(), user.getUrl_name()));
            if(array[3]>=1)
                pool.execute(new ThreadUserColumn(user.getUser_data_id(), user.getUrl_name()));
            if(array[4]>=1)
                pool.execute(new ThreadUserFollower(user.getUser_data_id(), user.getUrl_name()));
            if(array[5]>=1)
                pool.execute(new ThreadUserFollowee(user.getUser_data_id(), user.getUrl_name()));

            ZhihuUserAnswer zhihuUserAnswer = new ZhihuUserAnswer();
            if(array[1]>=1){
                for(int i=1 ;i<=array[1];i++){
                    pool.submit(new ThreadUserAnswer(user.getUser_data_id(),user.getUrl_name(),i, zhihuUserAnswer));
                }
            }
            pool.shutdown();
            //循环等待线程结束
            ...
            new Mongo().upsertUserAnswer(IDandName[0],zhihuUserAnswer);
            return "success";
        }else
            return "error";
    }
