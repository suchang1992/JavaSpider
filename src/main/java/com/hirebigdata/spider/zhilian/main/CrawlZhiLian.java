...
public class CrawlZhiLian {
    ...
    public void tryToLogin() throws IOException {
        while (true) {
            ...
            // validate
            String validate = getValidateCode(headerString);
    ...
    public File saveValidatePicture(HttpClient httpclient, String cookie) {
        try {
            ...
            Header header = getResponse.getHeaders("Set-Cookie")[0];
    ...
    public String getValidateCode(String cookie) {
        String line = "";
        while (line.length() < 4) {
            File f = saveValidatePicture(httpclient, cookie);
            try {
